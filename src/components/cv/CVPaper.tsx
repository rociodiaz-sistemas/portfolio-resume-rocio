...
const CVPaper: React.FC<PaperProps> = ({ children, ...props }) => {
  const [leftContent, rightContent] = React.Children.toArray(
    children
  ) as React.ReactNode[];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {leftContent}
        {!isFirstSet && (
          <FoldedCorner onFoldClick={prevSet} position="bottom-left" />
        )}
      </Box>
      ...
      >
        ...
        {rightContent}
        {!isLastSet && (
          <FoldedCorner onFoldClick={nextSet} position="bottom-right" />
        )}
      </Box>
    </Box>
  );
